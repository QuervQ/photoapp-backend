...
/// WebSocketアップグレードハンドラ。クエリまたはヘッダーからJWTを検証し、WS接続を確立。
pub async fn ws_handler(
    ...
    let user_id =
        extract_user_id_from_headers(&headers, &state.jwt_secret, query.token.as_deref())?;
...
/// WS接続のメインループ。ルームのbroadcast受信とクライアントメッセージを同時に処理。
...
/// クライアントからの受信メッセージを処理する。subscribeメッセージでルーム購読を切り替え。
